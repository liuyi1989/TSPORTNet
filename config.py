...
flags.DEFINE_integer('B', 16, 'number of capsules in output from PrimaryCaps')
...
#   environment setting    #
############################
flags.DEFINE_string('dataset', 'data/MSRA-B', 'the path for dataset')
flags.DEFINE_string('dataset_fashion_MSRA-B', 'data/fashion_MSRA-B', 'the path for dataset')
...
def get_dataset_size_train(dataset_name: str):
    options = {'MSRA-B': 3000, 'smallNORB': 23400 * 2,
               'fashion_MSRA-B': 55000, 'cifar10': 50000, 'cifar100': 50000}
    return options[dataset_name]


def get_dataset_size_test(dataset_name: str):
    options = {'MSRA-B': 10000, 'smallNORB': 23400 * 2,
               'fashion_MSRA-B': 10000, 'cifar10': 10000, 'cifar10': 10000}
    return options[dataset_name]


def get_num_classes(dataset_name: str):
    options = {'MSRA-B': 2, 'smallNORB': 5, 'fashion_MSRA-B': 10, 'cifar10': 10, 'cifar100': 100}
    return options[dataset_name]


from utils import create_inputs_MSRA_B, create_inputs_norb, create_inputs_cifar10, create_inputs_cifar100


#def get_create_inputs(dataset_name: str, f_img, f_label, is_train: bool, epochs: int):
#    options = {'MSRA-B': lambda: create_inputs_MSRA_B(f_img, f_label, is_train),
#               'fashion_MSRA-B': lambda: create_inputs_MSRA_B(is_train),
...
def get_create_inputs(dataset_name: str, f_img, f_label, is_train: bool, epochs: int):
    return create_inputs_MSRA_B(f_img, f_label, is_train)
