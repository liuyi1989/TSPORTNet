...
class Model:
    def __init__(self):
        ...
        self.S11_holder = tf.placeholder(tf.float32, [3*3*int(np.floor(cfg.B/2)), int(np.floor(cfg.C/2))])
        self.S12_holder = tf.placeholder(tf.float32, [3*3*int(np.floor(cfg.B/2)), int(np.floor(cfg.C/2))])
        self.S21_holder = tf.placeholder(tf.float32, [3*3*int(np.floor(cfg.C/2)), int(np.floor(cfg.D/2))])
        self.S22_holder = tf.placeholder(tf.float32, [3*3*int(np.floor(cfg.C/2)), int(np.floor(cfg.D/2))])
    ...
    def build_model(self):
        ...
        for i in range(2):
            with tf.variable_scope('conv_caps1'+str(i)) as scope:
                ...
                self.output1_i = tf.transpose(tf.reshape(self.output, shape=[cfg.batch_size*data_size*data_size, 1, 3*3*int(np.floor(cfg.B/2)), 17]), [0, 1, 3, 2])
                self.output1_i = tf.reshape( self.output1_i, [-1, 3*3*int(np.floor(cfg.B/2))])
                #self.output = tf.reshape(self.output, shape=[cfg.batch_size, data_size, data_size, 1*self.output.get_shape()[4]])
                ...
                with tf.variable_scope('routing'+str(i)) as scope:
                    #if i==1:
                        #miu, activation, _ = self.em_routing_s(votes, activation, int(np.floor(cfg.C/2)), self.S11_holder, weights_regularizer)
                    #else:
                        #miu, activation, _ = self.em_routing_s(votes, activation, int(np.floor(cfg.C/2)), self.S12_holder, weights_regularizer)
                    miu, activation, test2 = self.em_routing(votes, activation, int(np.floor(cfg.C/2)), weights_regularizer)
                    
                self.pose1 = miu
                self.activation1 = tf.reshape(activation, shape=[cfg.batch_size*data_size*data_size, 1, int(np.floor(cfg.C/2)), 1])
                self.output1_o = tf.concat([self.pose1, self.activation1], axis=3)
                self.output1_o = tf.transpose(self.output1_o, [0, 1, 3, 2])
                self.output1_o = tf.reshape(self.output1_o, [-1, int(np.floor(cfg.C/2))])
                self.output1_o_split = tf.split(self.output1_o, num_or_size_splits=int(np.floor(cfg.C/2)),axis=1)
                
                for j in range(int(np.floor(cfg.C/2))):                    
                    self.output1_o_tile = tf.tile(self.output1_o_split[j], [1, 3*3*int(np.floor(cfg.B/2))])                    
                    S1_temp = tf.reduce_sum(tf.square(self.output1_i-self.output1_o_tile), 0)
                    S1_temp = tf.reshape(S1_temp, [3*3*int(np.floor(cfg.B/2)), 1])
                    if j==0:
                        self.S1 = S1_temp
                    else:
                        self.S1 = tf.concat([self.S1, S1_temp], axis=1)
                        
                if i==0:
                    self.S11 = 1 - tf.nn.sigmoid(self.S1)
                else:
                    self.S12 = 1 - tf.nn.sigmoid(self.S1)
            ...
            with tf.variable_scope('conv_caps2'+str(i)) as scope:
                self.output = self.kernel_tile(self.output, 3, 1)
                self.output2_i = tf.transpose(tf.reshape(self.output, shape=[cfg.batch_size*data_size*data_size, 1, 3*3*int(np.floor(cfg.C/2)), 17]), [0, 1, 3, 2])
                self.output2_i = tf.reshape( self.output2_i, [-1, 3*3*int(np.floor(cfg.C/2))])
                #self.output = tf.reshape(self.output, shape=[cfg.batch_size, data_size, data_size, 1*self.output.get_shape()[4]])                
                #self.output = self.kernel_tile(self.output, 3, 1)
                #data_size = int(np.floor(data_size / 2))
                self.output = tf.reshape(self.output, shape=[cfg.batch_size *
                ...
                with tf.variable_scope('routing'+str(i)) as scope:
                    #if i==0:
                        #miu, activation, _ = self.em_routing_s(votes, activation, int(np.floor(cfg.D/2)), self.S21_holder, weights_regularizer)
                    #else:
                        #miu, activation, _ = self.em_routing_s(votes, activation, int(np.floor(cfg.D/2)), self.S22_holder, weights_regularizer)
                    miu, activation, test2 = self.em_routing(votes, activation, int(np.floor(cfg.D/2)), weights_regularizer)
                    
                self.pose2 = miu
                self.activation2 = tf.reshape(activation, shape=[cfg.batch_size*data_size*data_size, 1, int(np.floor(cfg.D/2)), 1])
                self.output2_o = tf.concat([self.pose2, self.activation2], axis=3)
                self.output2_o = tf.transpose(self.output2_o, [0, 1, 3, 2])
                self.output2_o = tf.reshape(self.output2_o, [-1, int(np.floor(cfg.D/2))])
                self.output2_o_split = tf.split(self.output2_o, num_or_size_splits=int(np.floor(cfg.D/2)),axis=1)
            
                for j in range(int(np.floor(cfg.D/2))):                    
                    self.output2_o_tile = tf.tile(self.output2_o_split[j], [1, 3*3*int(np.floor(cfg.C/2))])                    
                    S2_temp = tf.reduce_sum(tf.square(self.output2_i-self.output2_o_tile), 0)
                    S2_temp = tf.reshape(S2_temp, [3*3*int(np.floor(cfg.C/2)), 1])
                    if j==0:
                        self.S2 = S2_temp
                    else:
                        self.S2 = tf.concat([self.S2, S2_temp], axis=1)
            
                if i==0:
                    self.S21 = 1 - tf.nn.sigmoid(self.S2)      
                else:
                    self.S22 = 1 - tf.nn.sigmoid(self.S2)
                
                self.pose2 = miu
                self.activation2 = tf.reshape(activation, shape=[data_size*data_size, 1, int(np.floor(cfg.D/2)), 1])
                self.output2 = tf.concat([self.pose2, self.activation2], axis=3)                    
        ...
        with tf.variable_scope('class_caps') as scope:
            ...
            with tf.variable_scope('routing') as scope:
                miu, activation, test2 = self.em_routing(votes, self.activation_conv2, 2, weights_regularizer)
        ...
        self.output_split = tf.split(self.output, num_or_size_splits=2,axis=3)
        self.pre = self.output_split[0]
        
        self.fea_po_4 = self.Conv_2d(tf.concat([self.pre, self.Fea_P4_Concat], axis=3), [1, 1, (fea_dim + 1), fea_dim], 0.01, padding='VALID', name='pre_4_Concat')
        self.pre_3 = tf.image.resize_images(self.pre, [88, 88])
        self.fea_po_3 = self.Conv_2d(tf.concat([self.pre_3, self.Fea_P3_Concat], axis=3), [1, 1, (fea_dim + 1), fea_dim], 0.01, padding='VALID', name='pre_3_Concat')  
        self.pre_2 = tf.image.resize_images(self.pre_3, [176, 176])
        self.fea_po_2 = self.Conv_2d(tf.concat([self.pre_2, self.Fea_P2_Concat], axis=3), [1, 1, (fea_dim + 1), fea_dim], 0.01, padding='VALID', name='pre_2_Concat')
        self.pre_1 = tf.image.resize_images(self.pre_2, [352, 352])
        self.Score1 = self.Conv_2d(tf.concat([self.pre_1, self.Fea_P1_Concat], axis=3), [1, 1, (fea_dim + 1), 2], 0.01, padding='VALID', name='po_1_output_Score')
        
        self.fea_po_4_Up1 = tf.nn.relu(self.Deconv_2d(self.fea_po_4, [1, 88, 88, fea_dim], 5, 2, name='po_4_Deconv1'))
        self.fea_po_4_Up1_temp = self.Conv_2d(tf.concat([self.fea_po_4_Up1, self.fea_po_3], axis=3), [1, 1, fea_dim*2, fea_dim], 0.01, padding='VALID', name='po_4_Up1')
        self.fea_po_4_Up2 = tf.nn.relu(self.Deconv_2d(self.fea_po_4_Up1_temp, [1, 176, 176, fea_dim], 5, 2, name='po_4_Deconv2'))
        self.fea_po_4_Up2_temp = self.Conv_2d(tf.concat([self.fea_po_4_Up2, self.fea_po_2], axis=3), [1, 1, fea_dim*2, fea_dim], 0.01, padding='VALID', name='po_4_Up2')
        self.fea_po_4_Up3 = tf.nn.relu(self.Deconv_2d(self.fea_po_4_Up2_temp, [1, 352, 352, fea_dim], 5, 2, name='po_4_Deconv3'))
        self.Score4 = self.Conv_2d(self.fea_po_4_Up3, [1, 1, fea_dim, 2], 0.01, padding='VALID', name='po_4_output_Score')        
        
        
        self.fea_po_3_Up1 = tf.nn.relu(self.Deconv_2d(self.fea_po_3, [1, 176, 176, fea_dim], 5, 2, name='po_3_Deconv1'))
        self.fea_po_3_Up1_temp = self.Conv_2d(tf.concat([self.fea_po_3_Up1, self.fea_po_2], axis=3), [1, 1, fea_dim*2, 2], 0.01, padding='VALID', name='po_3_Score')   
        self.fea_po_3_Up2 = tf.nn.relu(self.Deconv_2d(self.fea_po_3_Up1_temp, [1, 352, 352, fea_dim], 5, 2, name='po_3_Deconv2'))
        self.Score3 = self.Conv_2d(self.fea_po_3_Up2, [1, 1, fea_dim, 2], 0.01, padding='VALID', name='po_3_output_Score')
        
        
        self.fea_po_2_Up1 = tf.nn.relu(self.Deconv_2d(self.fea_po_2, [1, 352, 352, fea_dim], 5, 2, name='po_2_Deconv1'))
        self.Score2 = self.Conv_2d(self.fea_po_2_Up1, [1, 1, fea_dim, 2], 0.01, padding='VALID', name='po_2_output_Score')        
        
            
            
            
        self.output_Up1 = tf.nn.relu(self.Deconv_2d(self.output, [1, 88, 88, fea_dim], 5, 2, name='output_Deconv1'))
        self.output_Up2 = tf.nn.relu(self.Deconv_2d(self.output_Up1, [1, 176, 176, fea_dim], 5, 2, name='output_Deconv2'))  
        self.output_Up3 = tf.nn.relu(self.Deconv_2d(self.output_Up2, [1, 352, 352, fea_dim], 5, 2, name='output_Deconv3'))   
            
        self.Score_caps = self.Conv_2d(self.output_Up3, [1, 1, fea_dim, 2], 0.01, padding='VALID', name='output_Score')      
        
        self.output_Score = self.Score_caps + self.Score1 + self.Score2 + self.Score3 + self.Score4
    ...
    def em_routing_s(self, votes, activation, caps_num_c, s, regularizer, tag=False):
        test = []
        ...
        #ind = tf.reshape(tf.greater_equal(s, 0.5), [1, s.get_shape()[0],s.get_shape()[1]])
        #s = tf.nn.softmax(s)
        ind = tf.reshape(s, [1, s.get_shape()[0],s.get_shape()[1]])
        ind = tf.tile(ind, [activation.get_shape()[0], 1, 1])
