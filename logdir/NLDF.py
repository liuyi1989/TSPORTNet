...
class Model:
    ...
    def build_model(self):
        ...
        with tf.variable_scope('primary_caps') as scope:
            ...
            data_size1 = int(np.floor(data_size / 2))
            data_size2 = int(np.floor(data_size1 / 2))
        ...
        for i in range(2):
            with tf.variable_scope('conv_caps1'+str(i)) as scope:
                self.output = tf.reshape(self.output_Primary_conv1[i], shape=[cfg.batch_size, data_size1*2, data_size1*2, -1])
                self.output = self.kernel_tile(self.output, 3, 2)
                self.output = tf.reshape(self.output, shape=[cfg.batch_size *
                                                                     data_size1 * data_size1, 3 * 3 * int(np.floor(cfg.B/2)), 17])
                activation = tf.reshape(self.output[:, :, 16], 
                                        shape=[cfg.batch_size * data_size1 * data_size1, int(np.floor(3 * 3 * cfg.B/2)), 1])
                
                activation_column = self.activation_trans_column(activation, data_size1, cfg.B)
                activation_row = self.activation_trans_row(activation, data_size1, cfg.B)
                activation = activation_column + activation_row
                
                #activation = tf.reshape(activation, shape=[data_size, data_size, 3 * 3 * int(np.floor(cfg.B/2)), 1])
                #activation = tf.transpose(activation, [3, 1, 2, 0])
                #activation = self.Conv_2d(activation, [1, 1, data_size, 1], 0.01, padding='VALID', name='activation'+str(i))
                #activation = tf.transpose(activation, [3, 1, 2, 0])
                #activation = tf.reshape(activation, shape=[1*data_size, 3 * 3 * int(np.floor(cfg.B/2)), 1])   
                ...
                with tf.variable_scope('v'+str(i)) as scope:
                    ...
                    votes_column = self.votes_trans_column(votes, data_size1, cfg.B, cfg.C)
                    votes_row = self.votes_trans_row(votes, data_size1, cfg.B, cfg.C)
                    votes = votes_column + votes_row
                    
                    #votes = tf.reshape(votes, shape=[data_size, data_size, 3 * 3 * int(np.floor(cfg.B/2)), int(np.floor(cfg.C/2))*16])
                    #votes = tf.transpose(votes, [3, 1, 2, 0])
                    #votes = self.Conv_2d(votes, [1, 1, data_size, 1], 0.01, padding='VALID', name='votes'+str(i))
                    #votes = tf.transpose(votes, [3, 1, 2, 0])
                    #votes = tf.reshape(votes, shape=[1*data_size, 3 * 3 * int(np.floor(cfg.B/2)), int(np.floor(cfg.C/2)), 16])
                ...
                with tf.variable_scope('routing'+str(i)) as scope:
                    ...
                    miu_column = self.miu_routing_trans_column(miu, data_size1, cfg.C)
                    miu_row = self.miu_routing_trans_row(miu, data_size1, cfg.C)
                    miu = miu_column + miu_row
                    
                    #miu = tf.reshape(miu, shape=[data_size, 1, int(np.floor(cfg.C/2)), 16])
                    #miu = tf.transpose(miu, [0, 3, 2, 1])
                    #miu = self.Conv_2d(miu, [1, 1, 1, data_size], 0.01, padding='VALID', name='votes_routing'+str(i))
                    #miu = tf.transpose(miu, [3, 0, 2, 1])          
                    
                    activation_column = self.activation_routing_trans_column(activation, data_size1, cfg.C)
                    activation_row = self.activation_routing_trans_row(activation, data_size1, cfg.C)
                    activation = activation_column + activation_row
                    #activation = tf.reshape(activation, shape=[data_size, 1, int(np.floor(cfg.C/2)), 1])
                    #activation = self.Conv_2d(activation, [1, 1, 1, data_size], 0.01, padding='VALID', name='activation_routing'+str(i))
                    #activation = tf.transpose(activation, [3, 0, 1, 2])
                    #activation = tf.reshape(activation, shape=[data_size*data_size, int(np.floor(cfg.C/2))])
                    
                    
                pose = tf.reshape(miu, shape=[cfg.batch_size, data_size1, data_size1, int(np.floor(cfg.C/2)), 16])
                activation = tf.reshape(
                            activation, shape=[cfg.batch_size, data_size1, data_size1, int(np.floor(cfg.C/2)), 1])   
                self.output = tf.reshape(tf.concat([pose, activation], axis=4), [
                                cfg.batch_size, data_size1, data_size1, -1])
                
            with tf.variable_scope('conv_caps2'+str(i)) as scope:
                self.output = self.kernel_tile(self.output, 3, 2)
                #data_size = int(np.floor(data_size / 2))
                self.output = tf.reshape(self.output, shape=[cfg.batch_size *
                                                                             data_size2 * data_size2, 3 * 3 * int(np.floor(cfg.C/2)), 17]) 
                activation = tf.reshape(self.output[:, :, 16], shape=[
                                    cfg.batch_size * data_size2 * data_size2, 3 * 3 * int(np.floor(cfg.C/2)), 1])
                         
                activation_column = self.activation_trans_column(activation, data_size2, cfg.C)
                activation_row = self.activation_trans_row(activation, data_size2, cfg.C)
                activation = activation_column + activation_row                
            
                with tf.variable_scope('v'+str(i)) as scope:
                    votes = self.mat_transform(self.output[:, :, :16], int(np.floor(cfg.D/2)), weights_regularizer)
                    
                    votes_column = self.votes_trans_column(votes, data_size2, cfg.C, cfg.D)
                    votes_row = self.votes_trans_row(votes, data_size2, cfg.C, cfg.D)
                    votes = votes_column + votes_row                    
            
                with tf.variable_scope('routing'+str(i)) as scope:
                    miu, activation, _ = self.em_routing(votes, activation, int(np.floor(cfg.D/2)), weights_regularizer)
                     
                    miu_column = self.miu_routing_trans_column(miu, data_size2, cfg.D)
                    miu_row = self.miu_routing_trans_row(miu, data_size2, cfg.D)
                    miu = miu_column + miu_row                    
                    
                    
                    activation_column = self.activation_routing_trans_column(activation, data_size2, cfg.D)
                    activation_row = self.activation_routing_trans_row(activation, data_size2, cfg.D)
                    activation = activation_column + activation_row
                                       
            
                pose = tf.reshape(miu, shape=[cfg.batch_size * data_size2 * data_size2, int(np.floor(cfg.D/2)), 16])
                activation = tf.reshape(
                                    activation, shape=[cfg.batch_size * data_size2 * data_size2, int(np.floor(cfg.D/2)), 1])
                if i==0:
        ...
        #with tf.variable_scope('class_caps') as scope:
            ...
            #self.output = tf.reshape(activation, shape=[
                                #cfg.batch_size, data_size2, data_size2, 2])
            
            
        self.output = tf.reshape(self.activation_conv2, shape=[cfg.batch_size, data_size2, data_size2, cfg.D*1])   
    ...
    def activation_trans_column(self, activation_input, data_size, cfg_num):
        activation = tf.reshape(activation_input, shape=[data_size, data_size, 3 * 3 * int(np.floor(cfg_num/2)), 1])
        activation = tf.transpose(activation, [3, 1, 2, 0])
        activation = self.Conv_2d(activation, [1, 1, data_size, 1], 0.01, padding='VALID', name='activation_column')
        activation = tf.transpose(activation, [3, 1, 2, 0])
        activation = tf.reshape(activation, shape=[1*data_size, 3 * 3 * int(np.floor(cfg_num/2)), 1])
        
        return activation
    
    def activation_trans_row(self, activation_input, data_size, cfg_num):
        activation = tf.reshape(activation_input, shape=[data_size, data_size, 3 * 3 * int(np.floor(cfg_num/2)), 1])
        activation = tf.transpose(activation, [0, 3, 2, 1])
        activation = self.Conv_2d(activation, [1, 1, data_size, 1], 0.01, padding='VALID', name='activation_row')
        activation = tf.transpose(activation, [0, 3, 2, 1])
        activation = tf.reshape(activation, shape=[1*data_size, 3 * 3 * int(np.floor(cfg_num/2)), 1])
        
        return activation    
    
    def votes_trans_column(self, votes_input, data_size, cfg_num1, cfg_num2):
        votes = tf.reshape(votes_input, shape=[data_size, data_size, 3 * 3 * int(np.floor(cfg_num1/2)), int(np.floor(cfg_num2/2))*16])
        votes = tf.transpose(votes, [3, 1, 2, 0])
        votes = self.Conv_2d(votes, [1, 1, data_size, 1], 0.01, padding='VALID', name='votes_column')
        votes = tf.transpose(votes, [3, 1, 2, 0])
        votes = tf.reshape(votes, shape=[1*data_size, 3 * 3 * int(np.floor(cfg_num1/2)), int(np.floor(cfg_num2/2)), 16])   
        
        return votes
    
    def votes_trans_row(self, votes_input, data_size, cfg_num1, cfg_num2):
        votes = tf.reshape(votes_input, shape=[data_size, data_size, 3 * 3 * int(np.floor(cfg_num1/2)), int(np.floor(cfg_num2/2))*16])
        votes = tf.transpose(votes, [0, 3, 2, 1])
        votes = self.Conv_2d(votes, [1, 1, data_size, 1], 0.01, padding='VALID', name='votes_row')
        votes = tf.transpose(votes, [0, 3, 2, 1])
        votes = tf.reshape(votes, shape=[1*data_size, 3 * 3 * int(np.floor(cfg_num1/2)), int(np.floor(cfg_num2/2)), 16])   
        
        return votes    
    
    def miu_routing_trans_column(self, miu_input, data_size, cfg_num):
        miu = tf.reshape(miu_input, shape=[1, data_size, int(np.floor(cfg_num/2)), 16])
        miu = tf.transpose(miu, [3, 1, 2, 0])
        miu = self.Conv_2d(miu, [1, 1, 1, data_size], 0.01, padding='VALID', name='miu_routing_column')
        miu = tf.transpose(miu, [3, 1, 2, 0])        
        
        return miu
    
    def miu_routing_trans_row(self, miu_input, data_size, cfg_num):
        miu = tf.reshape(miu_input, shape=[data_size, 1, int(np.floor(cfg_num/2)), 16])
        miu = tf.transpose(miu, [0, 3, 2, 1])
        miu = self.Conv_2d(miu, [1, 1, 1, data_size], 0.01, padding='VALID', name='miu_routing_row')
        miu = tf.transpose(miu, [0, 3, 2, 1])        
        
        return miu    
    
    def activation_routing_trans_column(self, activation_input, data_size, cfg_num):
        activation = tf.reshape(activation_input, shape=[1, data_size, int(np.floor(cfg_num/2)), 1])
        activation = tf.transpose(activation, [3, 1, 2, 0])
        activation = self.Conv_2d(activation, [1, 1, 1, data_size], 0.01, padding='VALID', name='activation_routing_column')
        activation = tf.transpose(activation, [3, 1, 2, 0]) 
        activation = tf.reshape(activation, shape=[data_size*data_size, int(np.floor(cfg_num/2))])   
        
        return activation
    
    def activation_routing_trans_row(self, activation_input, data_size, cfg_num):
        activation = tf.reshape(activation_input, shape=[data_size, 1, int(np.floor(cfg_num/2)), 1])
        activation = tf.transpose(activation, [0, 3, 2, 1])
        activation = self.Conv_2d(activation, [1, 1, 1, data_size], 0.01, padding='VALID', name='activation_routing_row')
        activation = tf.transpose(activation, [0, 3, 2, 1]) 
        activation = tf.reshape(activation, shape=[data_size*data_size, int(np.floor(cfg_num/2))])   
